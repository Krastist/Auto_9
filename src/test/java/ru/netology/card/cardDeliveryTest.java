...
public class cardDeliveryTest {
    ...
    private String getDateCalendar(int addDay) {
        LocalDate dateCalendar = LocalDate.now();
        LocalDate futureDateCalendar = dateCalendar.plusDays(addDay);
        DateTimeFormatter formatterCalendar = DateTimeFormatter.ofPattern("d");
        String formatDateCalendar = futureDateCalendar.format(formatterCalendar);
        return formatDateCalendar;
    }
    ...
    void validTest() {
        ...
        $("[data-test-id=notification] .notification__content").shouldHave(exactText("Встреча успешно забронирована на " + getDate(3)));
    }
    ...
    void validCalendarTest() {
        ...
        $$(".calendar__day").find(exactText(getDateCalendar(3))).click();
        $("[data-test-id=name] input").setValue("Иван Иванович");
        $("[data-test-id=phone] input").setValue("+71234567890");
        $("[data-test-id=agreement]").click();
        $("button.button").click();
